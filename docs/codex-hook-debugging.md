...
- appends newline-delimited JSON records under `~/.code-island-debug/codex-hooks/`
- forwards the same payload to `~/.code-island/bin/code-island-bridge --source codex` when that bridge launcher is available

Use `CODE_ISLAND_DEBUG_LOG_ONLY=1` or `--log-only` when you explicitly want capture-only mode with no bridge forwarding.

Example `~/.codex/hooks.json` snippet for temporary debugging:

```bash
REPO_ROOT="/absolute/path/to/code-island"
```
...
```bash
tail -n 20 ~/.code-island-debug/codex-hooks/$(date +%Y%m%d).jsonl
```

If you want to see only captured payloads without forwarding into Code Island, run the same command with `--log-only`:
