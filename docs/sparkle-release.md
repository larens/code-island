...
```bash
xcrun notarytool store-credentials "CodeIsland" \
...
- `scripts/create-styled-dmg.sh` now defaults to the repo-tracked installer artwork at `docs/images/code-island-dmg-installer-background.png`; set `CODE_ISLAND_DMG_BACKGROUND_SOURCE` if you need to preview a different background locally.
...
- `scripts/create-release.sh` packages `releases/notes/<version>.md` as `CodeIsland-<version>.md` and uses it as the GitHub Release body when present.
- `scripts/create-release.sh` infers the GitHub repo from `origin` by default; set `CODE_ISLAND_GITHUB_REPO=owner/repo` if you need to override it.
