...
WEBSITE_DIR="${CODE_ISLAND_WEBSITE:-$PROJECT_DIR/../CodeIsland-website}"
WEBSITE_PUBLIC="$WEBSITE_DIR/public"

APP_PATH="$EXPORT_PATH/Code Island.app"
APP_NAME="CodeIsland"
NOTARY_PROFILE="${CODE_ISLAND_NOTARY_KEYCHAIN_PROFILE:-CodeIsland}"
...
export CODE_ISLAND_RELEASE_DIR="$RELEASE_DIR"
export CODE_ISLAND_GENERATE_APPCAST=1
export CODE_ISLAND_NOTARY_KEYCHAIN_PROFILE="$NOTARY_PROFILE"
...
if ! command -v gh >/dev/null 2>&1; then
    ...
    echo "WARNING: Could not infer GitHub repository. Set CODE_ISLAND_GITHUB_REPO=owner/repo to enable release upload."
    echo "Skipping GitHub release."
else
...
- Download \`$(basename "$DMG_PATH")\` and install the latest Code Island release.
...
- Open the DMG, drag Code Island to Applications, and launch it normally.
- After installation, Code Island will automatically check for updates."
        fi
    fi
