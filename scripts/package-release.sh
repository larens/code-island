...
BUILD_DIR="${CODE_ISLAND_BUILD_DIR:-$PROJECT_DIR/build/release}"
DERIVED_DATA_PATH="$BUILD_DIR/DerivedData"
EXPORT_PATH="$BUILD_DIR/export"
RELEASE_DIR="${CODE_ISLAND_RELEASE_DIR:-$PROJECT_DIR/releases/signed}"
...
NOTARY_APPLE_ID="${CODE_ISLAND_NOTARY_APPLE_ID:-${APPLE_ID:-}}"
NOTARY_TEAM_ID="${CODE_ISLAND_NOTARY_TEAM_ID:-${APPLE_TEAM_ID:-}}"
NOTARY_PASSWORD="${CODE_ISLAND_NOTARY_PASSWORD:-${APPLE_APP_SPECIFIC_PASSWORD:-}}"
NOTARY_KEYCHAIN_PROFILE="${CODE_ISLAND_NOTARY_KEYCHAIN_PROFILE:-${NOTARY_KEYCHAIN_PROFILE:-CodeIsland}}"
SKIP_NOTARIZATION="${CODE_ISLAND_SKIP_NOTARIZATION:-0}"

SPARKLE_PRIVATE_KEY_PATH="${CODE_ISLAND_SPARKLE_PRIVATE_KEY_PATH:-$KEYS_DIR/eddsa_private_key}"
GENERATE_APPCAST="${CODE_ISLAND_GENERATE_APPCAST:-0}"
VALIDATE_RELEASE_PROGRESS="${CODE_ISLAND_VALIDATE_RELEASE_PROGRESS:-1}"
...
resolve_exported_app_icon() {
    local requested_icon_source="${CODE_ISLAND_DMG_ICON_SOURCE:-}"
    ...
    echo "$PROJECT_DIR/CodeIsland/Assets.xcassets/AppIcon.appiconset/icon_1024x1024.png"
}
...
infer_github_repo() {
    if [ -n "${CODE_ISLAND_GITHUB_REPO:-}" ]; then
        echo "$CODE_ISLAND_GITHUB_REPO"
        return 0
    fi
...
resolve_notary_credentials() {
    ...
    echo "Provide one of:"
    echo "  1. CODE_ISLAND_NOTARY_APPLE_ID / CODE_ISLAND_NOTARY_TEAM_ID / CODE_ISLAND_NOTARY_PASSWORD"
    echo "  2. A notarytool keychain profile named '$NOTARY_KEYCHAIN_PROFILE'"
    echo ""
    echo "You can skip notarization explicitly with CODE_ISLAND_SKIP_NOTARIZATION=1."
    exit 1
}
...
find_sparkle_bin_dir() {
    ...
    local candidates=(
        ...
        "$HOME/Library/Developer/Xcode/DerivedData/CodeIsland-*/SourcePackages/artifacts/sparkle/Sparkle/bin"
...
for release in releases:
    ...
    zip_url = next(
        (
            ...
            if asset.get("name", "").startswith("CodeIsland-")
...
PY
    ...
    unzip -p "$tmp_dir/previous.zip" "Code Island.app/Contents/Info.plist" > "$previous_plist"
...
echo "=== Packaging Signed Code Island ==="
echo ""

export CODE_ISLAND_BUILD_DIR="$BUILD_DIR"
export CODE_ISLAND_DMG_FAIL_ON_PLAIN=1
export CODE_ISLAND_DMG_BACKGROUND_SOURCE="$DMG_BACKGROUND_SOURCE"
"$SCRIPT_DIR/build.sh"
...
export CODE_ISLAND_DMG_ICON_SOURCE="$DMG_ICON_SOURCE"
...
create_styled_dmg "$APP_PATH" "$DMG_PATH" "Code Island" "$STAGING_DIR" "$PROJECT_DIR"
