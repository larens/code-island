...
struct IDEExtensionInstaller {
    ...
    static let extensionIdentifier = "code-island.session-focus"
    private static let extensionIconFilename = "icon.png"
    private static let extensionReadmeFilename = "README.md"
    private static let projectHomepage = "https://github.com/erha19/code-island"
    private static let projectRepository = "https://github.com/erha19/code-island.git"
    private static let projectIssues = "https://github.com/erha19/code-island/issues"
    ...
    private func packageJSON(sessionFocusStrategy: SessionFocusStrategy?) -> String {
        ...
        {
          "name": "session-focus",
          "displayName": "Code Island",
          "description": "\(description)",
          "version": "\(Self.extensionVersion)",
          "publisher": "code-island",
    ...
    private func extensionJS(sessionFocusStrategy: SessionFocusStrategy?) -> String {
            ...
            sessionFocusLogic = """
                async function focusChatSession(sessionId) {
                    ...
                    try {
                        ...
                        console.warn('[code-island] Failed to focus chat session', sessionId, error);
                        return false;
                    }
                }
            """
        ...
        function logInfo(message, details) {
            if (details === undefined) {
                console.info(`[code-island] ${message}`);
                return;
            }

            console.info(`[code-island] ${message}`, JSON.stringify(details));
        }

        function logWarn(message, details) {
            if (details === undefined) {
                console.warn(`[code-island] ${message}`);
                return;
            }

            console.warn(`[code-island] ${message}`, JSON.stringify(details));
        }
        ...
        async function handleSetupURI(uri) {
            ...
            await vscode.window.showInformationMessage('Code Island is ready.');
        }
    ...
    private func extensionReadme(sessionFocusStrategy: SessionFocusStrategy?) -> String {
        let capabilityLine = sessionFocusStrategy == .qoderChatHistory
            ...
            : "It reopens the matching terminal tab from Code Island's session context."

        return """
        # Code Island

        Code Island installs this VS Code-compatible extension so the app can jump back into the right IDE window for your active coding session.

        \(capabilityLine)

        Manage installs, reinstalls, and authorization from Code Island's **Settings -> Integration** panel.
    ...
    private static func applicationVersion() -> String {
        let projectURL = repositoryRootURL().appending(path: "CodeIsland.xcodeproj/project.pbxproj")
        guard
    ...
    private func repositoryAppIconURL() -> URL {
        Self.repositoryRootURL()
            .appending(path: "CodeIsland/Assets.xcassets/AppIcon.appiconset/icon_128x128.png")
    }

    private func extensionDescription(sessionFocusStrategy: SessionFocusStrategy?) -> String {
        sessionFocusStrategy == nil
            ? "Lets Code Island focus the matching terminal tab"
            : "Lets Code Island focus the matching chat session or terminal tab"
    }

    private func vsixManifest(sessionFocusStrategy: SessionFocusStrategy?) -> String {
        let description = sessionFocusStrategy == nil
            ? "Lets Code Island focus the matching terminal tab in VS Code compatible IDEs."
            : "Lets Code Island focus the matching chat session or terminal tab in VS Code compatible IDEs."

        return """
        <?xml version="1.0" encoding="utf-8"?>
        <PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011">
          <Metadata>
            <Identity Language="en-US" Id="session-focus" Version="\(Self.extensionVersion)" Publisher="code-island"/>
            <DisplayName>Code Island</DisplayName>
