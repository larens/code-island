...
struct HookInstaller {
    ...
    private static let supportDirectoryName = ".code-island"
    private static let bridgeLauncherName = "code-island-bridge"
    private static let bridgeBinaryName = "CodeIslandBridge"
    ...
    private static func preferredBridgeBinaryURL() -> URL? {
        let candidates = [
            Bundle.main.executableURL?
                .deletingLastPathComponent()
                .appendingPathComponent(bridgeBinaryName),
            URL(fileURLWithPath: "/Users/code-island/Island/Prototype/.build/debug/\(bridgeBinaryName)")
        ]
        .compactMap { $0 }
    ...
    private static func isManagedStatusLine(_ statusLine: [String: Any]?) -> Bool {
        ...
        return command == statusLineCommand()
            || command.contains("/.code-island/bin/\(statusLineScriptName)")
    }
    ...
    private static func isIslandManagedHookCommand(_ command: String) -> Bool {
        ...
        return normalized.contains("/.code-island/bin/code-island-bridge")
            || normalized.contains("/.code-island/bin/island-bridge")
    }

    private static func managedMarker(for profile: ManagedHookClientProfile) -> String {
        "Code Island managed integration: \(profile.id)"
    }
    ...
    static func managedPluginSource(for profile: ManagedHookClientProfile) -> String {
        ...
        return """
        // \(marker)
        // Generated by Code Island. Reinstall from Island settings if you need to refresh it.
        ...
        export const server = async ({ client, serverUrl }) => {
          ...
          return {
          name: "Code Island",
          description: "Forward OpenCode hook events to Code Island.",
          event: async ({ event }) => {
    ...
    static func managedPluginDirectoryFiles(for profile: ManagedHookClientProfile) -> [String: String] {
        ...
        name: code-island
        version: 1.0.0
        description: Forward Hermes Agent plugin hooks to Code Island
        provides_hooks:
          - on_session_start
          - pre_llm_call
          - pre_tool_call
    ...
    static func managedHookDirectoryFiles(
        ...
        ---
        name: \(hookName)
        description: "Forward OpenClaw internal hook events to Code Island"
        metadata:
        ...
        const handler = async (event) => {
          ...
          try {
            ...
            // OpenClaw hooks should never fail because Code Island is unavailable.
          }
        };
