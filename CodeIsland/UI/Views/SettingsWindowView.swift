...
struct SharedRuntimeLauncher: NativeRuntimeLaunching {
    func startSession(provider: SessionProvider, cwd: String) async throws -> NativeRuntimeLaunchResult {
        ...
        return NativeRuntimeLaunchResult(sessionID: handle.sessionID, statusMessage: nil)
    }
...
final class SettingsPanelViewModel: ObservableObject {
    ...
    func exportLogs() {
        ...
        panel.nameFieldStringValue = "CodeIsland-Diagnostics-\(Self.archiveTimestamp()).zip"
    ...
    private var sidebarSections: [SettingsSidebarSection] {
        [
            SettingsSidebarSection(
                title: nil,
                categories: [.general, .shortcuts, .display, .mascot, .sound, .integration, .about]
            )
        ]
    }
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            SettingsSectionCard(title: "链接") {
                SettingsActionLine(title: "GitHub", subtitle: "打开 Issues 页面反馈问题") {
                    if let url = URL(string: "https://github.com/erha19/code-island/issues") {
                        NSWorkspace.shared.open(url)
                    }
                } accessory: {
...
struct IslandSurfaceModeSelector: View {
    ...
    var subtitle: String? = "选择 Code Island 的主显示方式。你随时可以在设置里切换，并立即看到新的渲染效果。"
