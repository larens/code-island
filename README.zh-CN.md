<h1 align="center">
  <img src="docs/images/code-island-icon.svg" width="64" height="64" alt="Code Island 应用图标" valign="middle">&nbsp;
  Code Island
</h1>
<p align="center">
...
<p align="center">
  <a href="https://github.com/erha19/code-island/releases">
    <img src="https://img.shields.io/github/v/release/erha19/code-island?display_name=tag&style=flat-square" alt="最新版本">
  </a>
  <a href="https://github.com/erha19/code-island/releases">
    <img src="https://img.shields.io/github/downloads/erha19/code-island/total?style=flat-square" alt="Release 下载次数">
  </a>
...
<p align="center">
  <img src="docs/images/notch-panel.png" width="960" alt="Code Island 预览图">
</p>
<p align="center">
...
Code Island 关注的，是那些真正会打断编码节奏的时刻，并把它们用原生 macOS 刘海界面接住。
...
| <img src="CodeIsland/Assets.xcassets/CodexLogo.imageset/codex-logo.png" width="32" alt="Codex 产品图标"> | Codex App + Codex CLI | Codex app-server、hooks、rollout 解析兜底 | Codex 应用、终端 | 审批、输入请求、线程同步 |
| <img src="CodeIsland/Assets.xcassets/GeminiLogo.imageset/gemini-logo.png" width="32" alt="Gemini CLI 产品图标"> | Gemini CLI | Gemini CLI hooks（`~/.gemini/settings.json`） | 兼容终端宿主 | 会话生命周期、工具活动、通知、压缩前事件 |
| <img src="docs/images/mascots/hermes.gif" width="32" alt="Hermes Agent 宠物图标"> | Hermes Agent | 官方 plugin hooks（`~/.hermes/plugins/code_island/`） | Hermes CLI 终端宿主 | 用户输入、工具前后事件、回复完成、会话结束提示 |
...
| <img src="CodeIsland/Assets.xcassets/QoderLogo.imageset/qoder-logo.png" width="32" alt="Qoder 产品图标"> | Qoder/QoderWork/... | Qoder、QoderWork、Qoder CLI、JetBrains 兼容路径 | Qoder / QoderWork 窗口、终端 | 会话跳转、审批、提醒 |
...
SSH 远程支持是 Code Island 的正式能力，而不是额外脚本。它可以把桥接程序引导到远程 macOS / Linux 主机上，重写远程 Claude 兼容 hooks 和 Qwen Code hooks、安装受支持的 OpenClaw internal hooks，让事件先进入桥接层，再通过双向转发回到你本机的菜单栏 UI。因此即使会话跑在远程 SSH 终端里，审批、追问、通知和一键跳回也仍然能落在同一个 Island 界面里。
...
默认会使用仓库里的 `docs/images/code-island-dmg-installer-background.png` 作为 DMG 安装背景；如果你想在本地预览别的背景图，可以临时设置 `CODE_ISLAND_DMG_BACKGROUND_SOURCE`。
...
如果 `CodeIslandUITests-Runner` 在 macOS 上一直停在 suspended，优先在 Xcode 里用有效本地签名身份跑 UI 测试，并结合 `amfid` / `AppleSystemPolicy` 日志判断是不是代码签名或系统策略问题。

## 设置面板

Code Island 当前提供 4 个设置分类：
...
Code Island 在 `设置 -> Sound` 里提供三种声音模式：
...
Code Island 也会自动发现放在 `~/.openpeon/packs` 和 `~/.claude/hooks/peon-ping/packs` 下面的主题包。
...
主题包里的音频文件支持 `.wav`、`.mp3`、`.ogg`。如果当前主题包没有提供某个事件对应的分类，Code Island 会回退到该事件当前选中的 macOS 系统音。
...
- Claude 家族工具主要通过托管 hook 文件和内嵌的 `CodeIslandBridge` 启动器接入。
...
- 安装你希望 Code Island 监控的 CLI 或桌面客户端
